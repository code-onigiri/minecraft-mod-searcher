...
- [x] 1.2 (P) UI基盤（Tailwind CSS + DaisyUI）設定
...
- [x] 4.2 名寄せ（Identity Resolution）実装
  - Slug完全一致による統合ロジック
